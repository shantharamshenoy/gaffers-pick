...
function adminAuth(req, res, next) {
  ...
  if (!pwd || !checkAdminHash(pwd)) return res.status(401).json({ error: "Unauthorized" });
  next();
}
...
app.post("/api/admin/verify", (req, res) => {
  ...
  if (!password || !checkAdminHash(password)) return res.status(401).json({ ok: false });
  res.json({ ok: true });
});
...
app.post("/api/admin/groups", adminAuth, async (req, res) => {
  ...
  try {
    await pool.query("INSERT INTO groups (code, name) VALUES ($1, $2)", [code, name]);
    res.json({ code, name });
  } catch (e) {
...
app.get("/api/admin/groups", adminAuth, async (req, res) => {
  try {
    const { rows: groups } = await pool.query("SELECT * FROM groups ORDER BY created_at");
    const { rows: players } = await pool.query("SELECT group_code, name FROM players");
    const result = groups.map(g => ({
      ...g,
      players: players.filter(p => p.group_code === g.code).map(p => ({ name: p.name })),
    }));
    res.json(result);
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.get("/api/results", async (req, res) => {
  try {
    ...
    const map = {};
    rows.forEach(r => {
      map[r.match_id] = { homeScore: r.home_score, awayScore: r.away_score, yellows: r.yellows, reds: r.reds };
    });
    res.json(map);
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.post("/api/groups/:code/join", async (req, res) => {
  ...
  try {
    const { rows: groups } = await pool.query("SELECT * FROM groups WHERE code=$1", [code]);
    ...
    if (existing.length) {
      if (existing[0].pin !== pin) return res.status(401).json({ error: "Wrong PIN for that name" });
...
app.get("/api/groups/:code", async (req, res) => {
  try {
    const { rows } = await pool.query("SELECT code, name FROM groups WHERE code=$1", [req.params.code]);
...
app.post("/api/picks", async (req, res) => {
  const { groupCode, playerName, pin, matchId, homeScore, awayScore, yellows, reds } = req.body;
  try {
    // Verify PIN
    ...
    `, [groupCode, playerName, matchId, homeScore, awayScore, yellows ?? null, reds ?? null]);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.get("/api/picks/:groupCode/:playerName", async (req, res) => {
  ...
  try {
    ...
    if (!players.length || players[0].pin !== pin) return res.status(401).json({ error: "Invalid PIN" });
...
// ─── START ────────────────────────────────────────────────────────────────────
initDb().then(() => {
  app.listen(PORT, () => console.log(`Gaffer's Pick running on port ${PORT}`));
}).catch(err => {
